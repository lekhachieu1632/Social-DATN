                ...
                <div class="row px-2 ">
                    @foreach($data as $key => $value)
                        ...
                        @if($value['user_id_1'] == session(\App\Setting\Setting_Static::KEY.'-id'))
                            @php $user = \App\Models\UserInfo::where('user_id', $value['user_id_2'])->first()->toArray(); @endphp
                        @else
                    ...
                    <div class="suggestion-item-background mb-2 mr-2 ">
                        <div
                            style="
                                    ...
                                    position: relative;
                                ">
                            <a class="gapo-thumbnail gapo-thumbnail--1x1 mb-2 rounded" href="{{ route('post.list', $user['user_id']) }}"
                                style="
                                        background-image: url('{{ asset(\App\Setting\Setting_Dynamic::getImage(\App\Setting\Setting_Static::path_site_user, $user['avatar'])) }}');
                                    "></a><a
                                href="{{ route('post.list', $user['user_id']) }}">
                                <div
                            ...
                            <div class="button-container">
                                <button class="btn btn-block btn-sm btn-primary btn-width-90" >
                                    Bạn bè
                                </button>
                            </div>
                        </div>
