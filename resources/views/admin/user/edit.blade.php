                    ...
                    <div class="card-body row">
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <input type="text" class="form-control" id="userinfo_birthday"
                                   ...
                                   value="{{isset($data_info['birthday']) && $data_info['birthday'] ? date('d/m/Y',$data_info['birthday']) : old('birthday')}}"
                                   autocomplete="off">
                            @error('birthday')
                            <span class="text-danger">{{ $message  }}</span>
                            @enderror
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <select class="form-control" name="status">
                                @foreach(\App\Setting\Setting_Dynamic::GetStatus() as $key => $status)
                                    <option
                                        value="{{$key}}" {{ isset($data['type']) && $data['type'] == $key ? "selected":  ""}}>  {{ $status }}   </option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group col-md-6">
                            <label >Tiểu sử</label>
                            <textarea name="story"  class="form-control" placeholder="Hãy viết tiểu sửa bản thân vào đây" autocomplete="off">{{isset($data_info['story']) && $data_info['story'] ? $data_info['story'] : old('story')}}</textarea>
                            @error('story')
                            <span class="text-danger">{{ $message  }}</span>
                            @enderror
                        </div>
                        <div class="form-group col-md-6">
                            ...
                            <textarea name="introduce"  class="form-control" placeholder="Hãy giới thiệu bản thân" autocomplete="off">{{isset($data_info['introduce']) && $data_info['introduce'] ? $data_info['introduce'] : old('introduce')}}</textarea>
                            @error('introduce')
                            <span class="text-danger">{{ $message  }}</span>
                            @enderror
                        </div>
                        <div class="form-group col-md-6">
                            ...
                            <select class="form-control" name="relationship">
                                @foreach(\App\Setting\Setting_Dynamic::GetRelationship() as $key => $relationship)
                                    <option
                                        value="{{$key}}" {{ isset($data_info['relationship']) && $data_info['relationship']==$key ?"selected" : "" }}>  {{ $relationship }}   </option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group col-md-6">
                            ...
                            <select class="form-control" name="type">
                                ...
                                @foreach(\App\Setting\Setting_Dynamic::GetType() as $key => $type )
                                    <option
                                        value="{{$key }}" {{ (isset($data['type']) && $data['type'] ==$key ? "selected": "")}}>  {{ $type }}   </option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group col-md-6">
                            <label for="site_info_avatar " class="col-md-12">Ảnh đại diện</label>
                                ...
                                <div class="col-md-6">
                                    <div class="position-relative">
                                        <img src="{{ asset(\App\Setting\Setting_Dynamic::getImage($data_info['path'],$data_info['avatar'])) }}" alt="{{$data_info['avatar'] }}" class="img-fluid">
                                    </div>
                                </div>
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <div class="col-md-6">
                                <div class="position-relative">
                                    <img src="{{ asset(\App\Setting\Setting_Dynamic::getImage($data_info['path'],$data_info['image_cover'])) }}" alt="{{$data_info['image_cover'] }}" class="img-fluid">
                                </div>
                            </div>
                            <div class="input-group col-md-6">
                                <div class="custom-file">
